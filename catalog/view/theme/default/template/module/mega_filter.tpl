...
<?php
	...
	if( ! empty( $settings['refresh_results'] ) && $settings['refresh_results'] == 'using_button' && ! empty( $settings['place_button'] ) ) {
		...
		if( in_array( 'top', $place_button ) ) {
			$buttons['top'][] = sprintf( $button_temp, 'filter-btn-apply', '<i class="fa fa-check" aria-hidden="true"></i>Найти' );
		}
		
		if( in_array( 'bottom', $place_button ) ) {
			$buttons['bottom'][] = sprintf( $button_temp, 'filter-btn-apply', '<i class="fa fa-check" aria-hidden="true"></i>Найти' );
		}
	}
	
	if( ! empty( $settings['show_reset_button'] ) ) {
		$buttons['bottom'][] = sprintf( $button_temp, 'mfilter-button-reset', '<i class="fa fa-times" aria-hidden="true"></i>Сбросить');
	}
	
	if( ! empty( $settings['show_top_reset_button'] ) ) {
		$buttons['top'][] = sprintf( $button_temp, 'mfilter-button-reset', '<i class="fa fa-times" aria-hidden="true"></i>Сбросить'  );
	}
								...
								<div class="mfilter-options">
											...
											<?php if( $filter['type'] == 'related' ) { ?>
												<ul data-labels="<?php echo str_replace( '"', '&quot;', implode( '#|#', $filter['labels'] ) ); ?>">
													...
													<?php foreach( $filter['levels'] as $level_id => $level ) { ?>
														...
														<li>
															<div class="cat-filter-title"><?php echo $level['name']; ?></div>
															<select class="select2" data-type="category-<?php echo $filter['type']; ?>">
									...
									<?php } else if( $filter['type'] == 'search' || $filter['type'] == 'search_oc' ) { ?>
										<div class="mfilter-option mfilter-search<?php echo ! empty( $filter['button'] ) ? ' mfilter-search-button' : ''; ?>">
												<div class="search-title">Поиск по наванию</div>
											<input
