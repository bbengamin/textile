...
function historyUpdate(state){
    var href = window.location.href;
    var arr = href.split('#');
    history.pushState({data:'test'}, document.title, location.href);
    window.history.replaceState( {data:'test'} , document.title, arr[0] + state);
}
...
$(document).ready(function() {
    ...
    $('#callback-form').on('submit', function(e) {
        e.preventDefault();
        $('#callback-form input').removeClass('red_error');
        $('#callback-form .error').detach();
        $.ajax({
            url: 'index.php?route=checkout/fast/fastorder',
            type: 'post',
            data: $('#callback-form input'),
            dataType: 'json',
            success: function(json) {
                $('#callback-form .error');
                if (json['error']) {
                    if (json['error']['name']) {
                        $('#callback-form input[name="name"]').after('<span class="error">' + json['error']['name'] + '</span>');
                        $('#callback-form input[name="name"]').addClass('red_error');
                    }
                    if (json['error']['phone']) {
                        $('#callback-form input[name="phone"]').after('<span class="error">' + json['error']['phone'] + '</span>');
                        $('#callback-form input[name="phone"]').addClass('red_error');
                    }
                    if (json['error']['email']) {
                        $('#callback-form input[name="email"]').after('<span class="error">' + json['error']['email'] + '</span>');
                        $('#callback-form input[name="email"]').addClass('red_error');
                    }
                }
                if (json['success']) {
                    $('#callback-form').hide();
                    $('#callback-form').next().show();
                }
            }
        });
        return false;
